...
coefs <- readRDS(paste0('C:/Users/ngraetz/Dropbox/Penn/papers/cfr_migration/paper_figs/', file, '.RDS'))
...
main_coefs <- all_coefs[!(cov_name %in% c('LDI/pc growth rate','Urbanicity','Polity2','R squared')), ]

setcolorder(main_coefs, c('cov_name', 'All countries', 'North Africa and Middle East','Sub-Saharan Africa','Asia','Latin America and Caribbean','All countries - interaction'))

main_coefs[cov_name=='Growth rate (lag5-15-24)', cov_name := 'Population growth rate']
main_coefs[cov_name=='Log(LDI/pc)', cov_name := 'Income per capita']
main_coefs[cov_name=='Growth*Log(LDI/pc)', cov_name := 'Population growth rate * Income per capita']
main_coefs[cov_name=='EPR 15:24', cov_name := 'Employee-population-ratio']
main_coefs[cov_name=='LDI/pc gap', cov_name := 'Income per capita gap']

t <- kable(main_coefs, format = 'latex', booktabs = TRUE, caption='Coefficients from regional models and model of all countries.', col.names = c('',c('All countries','NAME','SSA','Asia','Latin America', 'All countries + Interaction')), digits = 2, escape = FALSE) %>%
...
```{r, echo=FALSE, eval=TRUE, results='asis'}
appendix_coefs <- all_coefs[!(cov_name %in% c('R squared')), ]

setcolorder(appendix_coefs, c('cov_name', 'All countries', 'North Africa and Middle East','Sub-Saharan Africa','Asia','Latin America and Caribbean','All countries - interaction'))

appendix_coefs[cov_name=='Growth rate (lag5-15-24)', cov_name := 'Population growth rate']
appendix_coefs[cov_name=='Log(LDI/pc)', cov_name := 'Income per capita']
appendix_coefs[cov_name=='Growth*Log(LDI/pc)', cov_name := 'Population growth rate * Income per capita']
appendix_coefs[cov_name=='EPR 15:24', cov_name := 'Employee-population-ratio']
appendix_coefs[cov_name=='LDI/pc gap', cov_name := 'Income per capita gap']

t <- kable(appendix_coefs, format = 'latex', booktabs = TRUE, caption='Coefficients from regional models and model of all countries.', col.names = c('',c('All countries','NAME','SSA','Asia','Latin America', 'All countries + Interaction')), digits = 2, escape = FALSE) %>%
   kable_styling(latex_options = c("striped","scale_down"), full_width = T, position = "center", font_size = 8) %>%
   add_footnote(c("* indicate parameters where p<=0.10.", "** indicate parameters where p<=0.05.", "All models include country fixed effects.")) %>%
   group_rows("", dim(appendix_coefs[!(cov_name %in% c('Adj. R squared', 'N')), ])[1] + 1, dim(appendix_coefs[!(cov_name %in% c('Adj. R squared', 'N')), ])[1] + 2) %>%
   column_spec(column = 1, width = "15em")
print(t)
```
...
```{r, echo=FALSE, eval=TRUE, results='asis'}
change_table <- readRDS('C:/Users/ngraetz/Dropbox/Penn/papers/cfr_migration/paper_figs/change_table.RDS')
change_table[, Increases := linebreak(Increases)]
change_table[, Decreases := linebreak(Decreases)]
kable(change_table, format = 'latex', col.names = c('','Positive\ncontributions','Negative\ncontributions'), caption = 'Total positive and negative contributions from growth.', booktabs = TRUE, digits = 2, escape = FALSE) %>%
   kable_styling(latex_options = c("striped","scale_down"), full_width = T, font_size = 8) %>%
   group_rows('Regions', 2, 13) %>%
   group_rows('World Bank Groups', 14, 17) %>%
...
```{r, echo=FALSE, eval=TRUE, results='asis'}
country_change_table <- fread('C:/Users/ngraetz/Dropbox/Penn/papers/cfr_migration/paper_figs/Figure_3_data.csv')
country_change_table <- country_change_table[1:20,]
country_change_table[, country_contribution := linebreak(country_contribution)]
kable(country_change_table, format = 'latex', col.names = c('','Positive\ncontributions'), caption = 'Total positive contributions from growth.', booktabs = TRUE, digits = 2, escape = FALSE) %>%
   kable_styling(latex_options = c("striped","scale_down"), full_width = T, font_size = 8)
```
